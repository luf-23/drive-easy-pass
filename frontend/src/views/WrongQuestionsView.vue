...
const totalPages = computed(() =>
  Math.max(1, Math.ceil(uniqueWrongQuestions.value.length / pageSize))
);
    ...
    <section class="panel wrong-book-panel">
      <div class="section-head">
        <div>
          ...
          <h2>
            {{ user?.nickname }} 当前共有
            {{ uniqueWrongQuestions.length }} 道错题
          </h2>
        </div>
        <button
          class="ghost"
          :disabled="uniqueWrongQuestions.length === 0"
      ...
      <div v-else class="wrong-list-scroll">
        <div class="wrong-list">
          <article
            v-for="item in pagedWrongQuestions"
          ...
          >
            ...
            <button class="danger" @click="removeWrongQuestion(item.id)">
              移除
            </button>
          </article>
        </div>
