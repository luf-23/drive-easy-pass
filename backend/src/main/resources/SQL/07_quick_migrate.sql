-- ========================================
-- 快速迁移脚本 - 删除旧表并使用 BCNF 新表结构
-- ========================================
-- 使用说明：
-- 1. 确保已经执行了 05_schema_bcnf.sql
-- 2. 如果没有历史数据，直接执行本脚本
-- 3. 如果有历史数据，请先执行 06_migration_script.sql
-- ========================================
...
-- 备份旧表（如果存在）
-- ========================================
-- 注意：如果有旧表数据需要保留，请先执行 06_migration_script.sql
-- 如果没有重要数据，可以直接删除旧表
-- 由于外键约束，需要先删除依赖表 enroll_follow_ups
...
-- 完成提示
-- ========================================
SELECT '✅ BCNF 优化完成！数据库表结构已更新。' AS message;
SELECT '⚠️ 请确保 Java 代码也已同步更新' AS reminder;
